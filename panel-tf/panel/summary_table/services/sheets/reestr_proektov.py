...
def _write(row: int, col: str, data):
    write_to_sheet(
        spreadsheet_id=spreadsheet_id, sheet='Реестр проектов', column=col, 
        row_id=row, data=data
    )

def _clear(start: str, finish: str):
    clear_sheet_range(
        spreadsheet_id=spreadsheet_id, sheet='Реестр проектов', start=start, 
        finish=finish
    )
...
def load_reestr_proektov():
    ...
    customerorders = get_documents(
        ...
        filters=[
            'applicable=true',
            f'state={reestr_proektov_customerorder_state}'
        ]
    )
