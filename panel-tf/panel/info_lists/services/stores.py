...
def _write(row: int, col: str, data):
    write_to_sheet(
        spreadsheet_id=spreadsheet_id, sheet='Склады', column=col, 
        row_id=row, data=data
    )

def _clear(start: str, finish: str):
    clear_sheet_range(
        spreadsheet_id=spreadsheet_id, sheet='Склады', start=start, 
        finish=finish
    )
...
def load_stores():
    start()
def load_stores_by_webhook():
    start()

def start():
    now = datetime.now()
    table = []
    stores = get_documents(
        doc_type='store', 
        filters=[
            'archived=false'
        ]
    ...
    for store in stores:
        table.append([])
        table[len(table) - 1].append(store['name'])
    table.sort()
