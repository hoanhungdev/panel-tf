...
from moysklad.services.reports.stock import get_report_by_doc_id
from bonuses.services.base import spreadsheet_id, b2_sheet_id, summary_sheet_id, \
...
def _write(row: int, col: str, data):
    write_to_sheet(
        spreadsheet_id=spreadsheet_id, sheet='Бонус 2', column=col, 
        row_id=row, data=data
    )

def _clear(start: str, finish: str):
    clear_sheet_range(
        spreadsheet_id=spreadsheet_id, sheet='Бонус 2', start=start, 
        finish=finish
    )
...
def start(dates={}):
    ...
    demands = get_documents(
        ...
        filters=[
            ...
            f'moment<{date_to.year}-{date_to.month}-{date_to.day} 00:00:00',
        ]
    )
    
    for id, dem in enumerate(demands):
        dem = get_document_by_id(
            doc_type='demand', doc_id=dem['id'], 
            attributes=['expand=project, agent, organization, contract, state, positions'],
        )
        table.append([])
        # дата отгрузки
        ...
        # № отгрузки
        table = _add(_get_link(doc=dem, type='demand', text=chained_get(dem, keys=['name'])))
        # организация
        ...
        table = _add(agent)
        # проект
        project = chained_get(dem, keys=['project'])
        ...
        # себестоимость
        cost = 0
        for pos in get_report_by_doc_id(doc_id=dem['id'])['positions']:
            cost += pos['cost']
        cost = format_sum(cost)
        table = _add(cost, type='sum')
        # статус отгрузки
        contract_state = chained_get(dem, keys=['state', 'name'])
        ...
        try:
            ...
            fact = Decimal(0.0)
        fact = fact.quantize(Decimal("1.00"))
        table = _add(fact, type='sum')
        # статус договора
        contract_state = chained_get(dem, keys=['contract', 'state', 'name'])
        table = _add(contract_state)
        # сумма договора
        contract_sum = chained_get(dem, keys=['contract', 'sum'])
        ...
        prepaid = search_attr_value_by_id(
            attributes=chained_get(dem, keys=['contract', 'attributes']),
            id=attr_id
        )
        table = _add(prepaid)
        ...
        # перемещение ндс
        attr_id = 'df2a6503-d30c-11e9-0a80-0181001571dd'
        nds = search_attr_value_by_id(
            attributes=chained_get(dem, keys=['project', 'attributes']),
            id=attr_id
        )
        nds_numeric = 0
        if nds:
            nds_numeric = 1
        table = _add(nds_numeric)
        # бонус 2 начислен
        attr_id = 'f2d87dfb-d0b4-11e9-0a80-01810000490c'
    ...
    header = [[
        'Дата отгрузки','№ отгрузки', 'Организация', 'Контрагент', 'Проект', 
        'Сумма отгрузки', 'Оплачено',
        'Себестоимость', 'Статус отгрузки', 'Факт наценка', 'Статус договора', 
        'Сумма договора', 'Сумма аванса', 'Карточка проекта утверждена',
        'Группа проекта', 'План.наценка', 'Перемещение НДС', 
        'Бонус 2 начислен', 'КВ', 'ПП', 'ПЗ', 'ПР', 'СП', 'ФП', 'КМ', 'РП', 'СМР'
    ]]
    table = header + table
    
    _clear(start='A7', finish='AA1000')
    _write(row=3, col='A', data=h2)
    _write(row=4, col='A', data=h1)
